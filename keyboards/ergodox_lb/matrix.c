...
static matrix_row_t read_cols(uint8_t row) {
  if (row < 7) {
    if (mcp23018_status) {  // if there was an error
      ...
      mcp23018_status = i2c_start(I2C_ADDR_READ, ERGODOX_LB_I2C_TIMEOUT);
      if (mcp23018_status) goto out;
      mcp23018_status = i2c_read_nack(ERGODOX_LB_I2C_TIMEOUT);
...
static void select_row(uint8_t row) {
  if (row < 7) {
    ...
    if (mcp23018_status) {  // if there was an error
      ...
      if (mcp23018_status) goto out;
      mcp23018_status = i2c_write(GPIOA, ERGODOX_LB_I2C_TIMEOUT);
      if (mcp23018_status) goto out;
      mcp23018_status = i2c_write(0xFF & ~(1 << row), ERGODOX_LB_I2C_TIMEOUT);
      if (mcp23018_status) goto out;
    out:
      i2c_stop();
    }
